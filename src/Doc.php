      ...
      dump($this->config);
        ...
        foreach ($controller as $k => $class) {
            ...
            if (class_exists($class)) {
                ...
                $filter_method = array_merge(['__constructx'], $this->config['filter_method']);
                ...
                foreach ($method as $key => $action) {
                    if (!in_array($action->name, $filter_method) && $action->class === $class) {
                        if ($doc->parse_action($action)){
                            ...
                            $tmp = $doc->parse_action($action);
                            
                            if(@$tmp['title'] == '') $tmp['title'] = $tmp['url'];
                            
                            $list[$k]['action'][$key] = $tmp;
                            //dump($doc->parse_action($action));exit();
                        }
                    }
                }
            }
